 ...
 */
float
REGUL_IntegralBackStep (REGUL_integ_back_step_s *pStruct,
                        float e1,
                        float phi_d,
                        float omega_x)
{
 ...
 */
float
REGUL_PI_regulator (REGUL_pid_regulator_s *pSturct, float error)
{
  float propCorrect;
  float piCorrect;
...
REGUL_PIDRegulator (
                    ...
                    float derivError)
{
  float returnValue = error * pData_struct->kp;

  pData_struct->integral += error * pData_struct->ki;

  if (error < pData_struct->integralSaturation)
    {
      pData_struct->integral
              = RestrictionSaturation
              (pData_struct->integral,
               pData_struct->integralSaturation);

      returnValue += pData_struct->integral;
    }
  
  returnValue += derivError * pData_struct->kd;
  
  returnValue = RestrictionSaturation (returnValue, pData_struct->saturation);
